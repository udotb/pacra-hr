        ...
        <div class="content container-fluid">
                ...
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <a type="button" style="display: flex; float: right"
                           class="btn-success btn" href="{{url('reimbursement-form')}}">Apply For Reimbursement</a>
                        <ul class="breadcrumb">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped custom-table mb-0 datatable"
                            ...
                            @foreach($reimbursements as $index=>$reimbursement)
                                <tr>
                                    <td>{{$index+1}}</td>
                                    <td style="white-space: nowrap">{{$reimbursement->display_name}}</td>
                                    {{--                                    <td>{{$reimbursement->type}}</td>--}}
                                    <td style="white-space: nowrap"
                                        title="{{ str_replace($spam, '', $reimbursement->type)}}">{{ str_replace($spam, ' ', $reimbursement->type) }}</td>
                                    <td>{{$numOfDays}}</td>
                                    <td>{{($reimbursement->client == 'N/A')?($reimbursement->client):($reimbursement->other_client)}}</td>

                                    <td style="white-space: nowrap">{{ date('d-M-Y', strtotime($reimbursement->dated))}}</td>
                                    <td>Rs. {{$reimbursement->amount}}</td>
                                    <td>{{$reimbursement->status}}</td>
                                    <td>
                                        <a data-target="#approval_modal{{$reimbursement->id}}" data-toggle="modal"
                                           href="#">
                                        ...
                                        <a title="Edit" target=_blank
                                           href="{{ url('edit-reimbursement-form'). '/' .$reimbursement->id }}">
                                            <i title="Edit">
                                                <svg width="20px" height="20px" viewBox="0 0 1792 1792"
                                ...
                                <div class="modal custom-modal fade" id="approval_modal{{$reimbursement->id}}"
                                     role="dialog">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-body">
                                                <div class="form-header">
                                                    <h3>{{$reimbursement->display_name}}</h3>
                                                </div>
                                                <div class="modal-btn delete-action">
                                                    <div class="form-group">
                                                        <h4 style="padding-top: 30px">Client</h4>
                                                        {!! ($reimbursement->client == 'N/A')?($reimbursement->client):($reimbursement->other_client) !!}
                                                        <h4 style="padding-top: 30px">Type Of Reimbursement</h4>
                                                        {!! str_replace($spam, ' ', $reimbursement->type) !!}
                                                        @if($reimbursement->TravelType)
                                                            <h4 style="padding-top: 30px">Type Of Travel</h4>
                                                            {!! $reimbursement->TravelType ?? '' !!}
                                                        @endif
                                                        @if($reimbursement->MedType)
                                                            <h4 style="padding-top: 30px">Type Of Medical</h4>
                                                            {!! $reimbursement->MedType ?? '' !!}
                                                        @endif
                                                        <h4 style="padding-top: 30px">From Date</h4>
                                                        {!! date('d-M-Y', strtotime($reimbursement->from_date)) !!}
                                                        <h4 style="padding-top: 30px">To Date</h4>
                                                        {!! date('d-M-Y', strtotime($reimbursement->to_date)) !!}
                                                        <h4 style="padding-top: 30px">Description</h4>
                                                        {!! $reimbursement->description !!}
                                                        <h4 style="padding-top: 30px">Amount</h4>
                                                        <p>Rs: {{$reimbursement->amount}}</p>
                                                        @if($reimbursement->user_id == $userId && $reimbursement->decline_reason != null)
                                                            <h4 style="padding-top: 30px">Decline Reason</h4>
                                                            <p>{{$reimbursement->status}}</p>
                                                            <p style="color: indianred; font-size: 26px">{{$reimbursement->decline_reason}}</p>
                                                        @endif
                                                    </div>
                                                </div>
                                                {{--                                                <div>--}}
                                                {{--                                                    @if($userId == $user[0]->am_id)--}}
                                                {{--                                                        <button class="btn btn-success btn-sm" value="recommend">--}}
                                                {{--                                                            Recommend--}}
                                                {{--                                                        </button>--}}
                                                {{--                                                        <button class="btn btn-danger btn-sm" value="decline-tl">--}}
                                                {{--                                                            Decline--}}
                                                {{--                                                        </button>--}}
                                                {{--                                                    @elseif(\Illuminate\Support\Facades\Auth::id() == 5)--}}
                                                {{--                                                        <button class="btn btn-success btn-sm" value="approve">Approve--}}
                                                {{--                                                        </button>--}}
                                                {{--                                                        <button class="btn btn-danger btn-sm" value="decline-acc">--}}
                                                {{--                                                            Decline--}}
                                                {{--                                                        </button>--}}
                                                {{--                                                    @endif--}}
                                                {{--                                                </div>--}}
            ...
            @if($totalAmount)
                <div style="display: flex; float: right; padding-top: 20px; padding-right: 10px">
                    <h3 id="udu" style="color: indianred">Pending Rs: {{$totalAmount}}</h3>
                </div>
            @endif
        </div>
    </div>
